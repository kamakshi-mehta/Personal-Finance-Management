...
import { Landmark, TrendingUp, Calendar, ArrowUpRight, Plus, Trash2, Lightbulb, Loader2, AlertCircle } from 'lucide-react';
...
const MutualFunds = () => {
  ...
  const fetchSips = async () => {
    ...
    setError('');
    try {
      const res = await axiosClient.get('/investments', { params: { type: 'mutual_fund' } });
      setActiveSips(res.data);
    } catch (err) {
      console.error('Error fetching SIPs:', err.message);
      setError('Failed to fetch SIP records from database');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddSip = async (e) => {
    ...
    try {
      await axiosClient.post('/investments', {
        ...
        interestRate: yieldPercent ? parseFloat(yieldPercent) : undefined,
        maturityDate: nextDate ? new Date(nextDate) : undefined
      });
      ...
      console.error('Error saving SIP:', err.message);
      setError('Failed to save Mutual Fund SIP record');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteSip = async (id) => {
    ...
    try {
      ...
      console.error('Error deleting SIP:', err.message);
      setError('Failed to delete SIP record');
    }
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex items-center justify-center p-12 bg-white rounded-2xl border border-slate-200">
          ...
          <span className="text-slate-500 text-sm font-semibold">Loading SIP portfolios...</span>
        </div>
      ) : (
        <>
          {/* Metrics Grid */}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {activeSips.length === 0 ? (
                  ...
                  activeSips.map((sip) => (
                    <tr key={sip._id} className="table-row">
                      ...
                      <td className="p-4 text-sm text-slate-400">
                        {sip.maturityDate ? new Date(sip.maturityDate).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : 'TBD'}
                      </td>
                      <td className="p-4 text-sm font-semibold text-blue-600">
                        {sip.interestRate ? `+${sip.interestRate}% annualized` : '+12.0% annualized'}
                      </td>
